...
EMT_RAW_GZ="${IMAGE_NAME}.raw.gz"
EMT_SHA256SUM="${IMAGE_NAME}.raw.gz.sha256sum"

curl -k --noproxy "" ${FILE_RS_URL}/files-edge-orch/repository/microvisor/${EMT_RAW_GZ} -o edge_microvisor_toolkit.raw.gz
curl -k --noproxy "" ${FILE_RS_URL}/files-edge-orch/repository/microvisor/${EMT_SHA256SUM} -o edge_microvisor_toolkit.raw.gz.sha256sum
