...
class EdgeMicroVisorToolKitStandaloneNodeDeployment(object):
    ...
    def run_lsblk(self):
        try:
            ...
            if result.returncode == 0:
                ...
                print(colored("Error running lsblk:", result.stderr, "red"))
                return None
    ...
    def install_standalone_edge_node_setup_script(self):
        ...
        try:
            ...
            command = (
                f"sudo ./bootable-usb-prepare.sh {disk}"
                " usb-bootable-files.tar.gz"
                " config-file > usb_preparation.log 2>&1"
            )
            ...
            if result.returncode == 0:
                ...
                self.log.error(f"Standard Error: {result.stderr}")
                print(colored(
            ...
            self.log.error(f"An error occurred during the installation: {e}")
            print(colored(
                f"An error occurred during the installation: {e}", "red"
            ))
