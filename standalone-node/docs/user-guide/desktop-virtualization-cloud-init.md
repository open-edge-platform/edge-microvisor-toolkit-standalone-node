...
write_files:
  ...
  - path: /etc/cloud/network_config.sh
    permissions: '0755'
    content: |
      #!/bin/bash
      ...
      # Validate the bridge name format
      br_check_bridge_name() {
          local bridge_name="$1"
          if ! [[ "$bridge_name" =~ ^[a-zA-Z0-9_-]+$ ]]; then
              echo "Invalid bridge name: $bridge_name. Only alphanumeric characters, underscores, and hyphens are allowed."
              exit 1
          fi
      }
      ...
      br_check_custom_network_config() {
          ...
          if ! grep -qE 'BRIDGE_CIDR|START_RANGE|END_RANGE|BRIDGE_GATEWAY|DNS_NAMESERVER' "$config_file"; then
      ...
      br_usage() {
          echo "Usage: $0 <bridge_name> <custom_network.conf>"
          echo "Example: $0 br0 custom_network.conf"
      }
      ...
      br_main() {
          ...
          if [[ $# -eq 2 ]]; then
              ...
              br_check_root
              br_check_dependencies
              br_check_bridge_name "$BR_NAME"
              br_check_custom_network_config "$2"
              br_modprob_br_netfilter
              br_parse_custom_network_config "$2"
              if [[ -z "$BR_CIDR" ]]; then
                  echo "BR_CIDR is not set in the configuration file."
                  exit 1
              fi
...
runcmd:
  ...
  - sudo bash /etc/cloud/network_config.sh br0 /etc/cloud/custom_network.conf
