...
write_files:
  ...
  - path: /etc/cloud/apply_bridge_nad.sh
    permissions: '0755'
    content: |
        #!/bin/bash
        ...
        check_nad_crd() {
            if kubectl get crd networkattachmentdefinitions.k8s.cni.cncf.io >/dev/null 2>&1; then
                return 0
            else
                return 1
            fi
        ...
        main() {
            ...
            if [[ $# -eq 1 ]]; then
                ...
                # Wait for K3s (or kubectl) and NetworkAttachmentDefinition CRD to be available
...
runcmd:
  ...
  - sudo -E bash /etc/cloud/apply_bridge_nad.sh /etc/cloud/custom_network.conf > /etc/cloud/apply_bridge_nad.log &
