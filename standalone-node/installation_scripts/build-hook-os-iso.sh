...
tar -czf usb-bootable-files.tar.gz hook-os.iso "$os_filename" sen-k3s-package.tar.gz $checksum_file > /dev/null

if tar -czf usb-bootable-files.tar.gz hook-os.iso "$os_filename" sen-k3s-package.tar.gz $checksum_file > /dev/null; then
